...
interface BrandRequest {
  featured_banner: Brand['featured_banner']
}
...
export default function BrandsEditFeaturedBanner ({ brand, open, onOpenChange }: Props): ReactNode {
  const { control, handleSubmit, formState: { isDirty }, reset } = useForm<BrandForm>({
    defaultValues: {
      featured_banner: [
        {
          url: brand.featured_banner ?? '',
          name: '',
          size: 0
        }
      ]
  ...
  return (
    <form>
      <Drawer open={open} onOpenChange={onOpenChange}>
        <Drawer.Content>
          <Drawer.Header>
            <Drawer.Title>Edit Brand Banner</Drawer.Title>
          </Drawer.Header>
...
interface ThumbnailProps {
  image: FieldArrayWithId<BrandForm, 'featured_banner', 'id'>
  index: number
